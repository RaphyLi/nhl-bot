...
export class StandingService {
  ...
  get(command?: SlashCommand): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      fetch<Standings>(
        this.BASE_URL +
          `/standings${options ? '?' + qs.stringify(options, { indices: false }) : ''}`
      ).then((result) => {
  ...
  private formatLine(teams: Array<TeamRecord>): string {
    ...
    teams.forEach((team) => {
      ...
      const name =
        `${team.divisionRank}. ` +
        team.team.teamName +
        leftPad.substring(team.team.teamName.length, leftPad.length);
      message +=
        `${name}` +
        `${this.formatNumber(team.gamesPlayed)} ${this.formatNumber(
          team.leagueRecord.wins
        )} ${this.formatNumber(team.leagueRecord.losses)}  ${this.formatNumber(
          team.leagueRecord.ot
        )}` +
        `  ${this.formatNumber(team.points, true)}   ${this.formatNumber(
